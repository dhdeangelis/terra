...
//geos
		...
		SpatVector buffer2(std::vector<double> d, size_t quadsegs);
		SpatVector buffer3(std::vector<double> d, size_t quadsegs);
...
//		SpatVector bufferclip(std::vector<double> d, std::string jointype, double miter_limit, int precision, double arc_tolerance);
		...
		SpatVector buffer(std::vector<double> d, size_t quadsegs, std::string capstyle, std::string joinstyle, double mitrelimit, bool singlesided);
		SpatVector buffer_lonlat(std::string vt, std::vector<double> d, size_t quadsegs);


		SpatVector point_buffer(std::vector<double>	d, size_t quadsegs, bool no_multipolygons, bool wrap);
